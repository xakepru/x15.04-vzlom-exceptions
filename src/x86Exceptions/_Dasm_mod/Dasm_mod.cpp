...
int Disasm (void * pCode)
{
	...
	__asm
	{
		...
		xa_nxtIndx:			//*xor al,al
			...
			_emit 0xd6 //salc			// if bit==0 then al=0 (salc==sbb al,al but not change flags)
		...
		xa_NxtByte:
			...
			cmove	ebx, ecx	// ... ebx=ecx=0 (current mode - 16 bits) *jne n32//xor ebx,ebx//n32:
			cmp	al, 67h		// if 67h present then ...
			cmove	edx, ecx	// ... edx=ecx=0 (current addressing mode - 16 bits)
			cmp	al, 0EAh	// JMP FAR
			je	xa_jmp
		...
		xa_Nrm:	pushfd			// store FLAGS
			//aam	//10h		//*mov ah,al//shr ah,4//and al,0Fh
			_emit 0xd4
			_emit 0x10
		...
		xa_ExtGroup:
			...
			jecxz	xa_3
		xa_1:	bt	[edi], ebp
			jnc	xa_2		// is not ModR/M only line?
			inc	edx		// yeah
		xa_2:	inc	ebp
			loop	xa_1
			jc	xa_3
			_emit 0xd6//salc			//*xor al,al
			cdq			//*xor edx,edx
		xa_3:	shl	edx, 1
			jmp	xa_ProcOpcode
		// ==========[ normal group ]==========
		xa_NrmGroup:
			...
			and	al, 7
		xa_4:	jecxz	xa_4x
		xa_5:	adc	dl, 1		// in first pass CF==0
			inc	ebp
			bt	[edi], ebp
			loop	xa_5
			jc	xa_ProcOpcode
		...
		xa_noSIB:
			...
			jc	xa_iWD		// Mod==10
			js	xa_i8		// Mod==01
			cmp	al, 5		// if (RM==5||BASE==5) && Mod==00 then ... (32 bit)
			jne	xa_enModRM	// if            RM==6 && Mod==00 then ... (16 bit)
...
// ------------------------------------------------------
// Function tries to find RET op-code
// ------------------------------------------------------

int GetFuncSize (unsigned char* pCode)
{
	unsigned long CLength, dwCount = 0;

	for (;;)
	{
		CLength	= Disasm((void*)pCode);
		
		dwCount		+= CLength;
		
		if ((*(unsigned char*)pCode == 0xC3) || (*(unsigned char*)pCode == 0xC2)) break;
...
// ------------------------------------------------------
// Get minimal amount of bytes required to store instr
// ------------------------------------------------------

int GetMinSize (unsigned char* pCode, unsigned long dwRequestedSize)
{
	unsigned long MinLength = 0, CLength;
	...
	} while(MinLength < dwRequestedSize);
